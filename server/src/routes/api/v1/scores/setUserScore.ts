...
export default function postSetUserScore(app: express.Application) {
    app.post('/api/v1/scores/setUserScore/:targetId', async (req, res) => {
        ...
        if (!(user.assistantOfTroopId || user.leaderOfTroopId || user.leaderOfPatrolId)) {
            res.sendStatus(403);
            return;
        }
        ...
        if (target.assistantOfTroopId || target.leaderOfTroopId) {
            res.sendStatus(400);
            return;
        }
        ...
        if (user.leaderOfPatrolId && (target.patrolId !== user.leaderOfPatrolId)) {
            res.sendStatus(403);
            return;
        }

        const task = await prisma.task.findUnique({ where: { id: taskId, taskTemplate: { individualTask: true } }, include: { taskTemplate: true } });
        if (!task) {
            res.sendStatus(400);
            return;
        }
        ...
        const taskScore = await prisma.userTaskScore.findFirst({
            where: {
                userId: targetId,
                taskId: taskId
            }
        ...
        if (taskScore) {
            ...
            const updatedTaskScore = await prisma.userTaskScore.update({
                where: {
                    id: taskScore.id
                },
                data: {
        ...
        const newTaskScore = await prisma.userTaskScore.create({
            data: {
                taskId: taskId,
                userId: targetId,
                score: score,
