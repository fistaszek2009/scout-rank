...
import { getUserInfo, getEventInfo, deletePatrol, getTroopInfo } from "@/utils/requests";
import { getSessionInfo } from "@/utils/session";
import CustomButton from "@/comp/CustomButton"
import PatrolItem from "@/comp/PatrolItem";
...
export default function Patrols() {
  ...
  const deletePatrolProc = async () => {
    await deletePatrol(toDeleteIdx);
    updateData();
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-400 flex-col items-center gap-5">
      <CustomButton text="Dodaj zastęp" className="w-96 items-center mt-5 py-5 px-10 bg-slate-600 rounded-2xl" textClassName="text-2xl text-slate-200" onPress={() => router.push('/(main)/admin/patrols/addPatrol')} />
      <FlatList data={troopInfo?.patrols} renderItem={({ item }) => {
        return <PatrolItem onLongPress={() => { setToDeleteIdx(item.id); setModalVisible(true) }} patrol={item} />
      }}/>
      <Modal
        animationType="slide"
        transparent={true}
        ...
        <View className="flex-1 justify-center items-center">
          <View className="bg-slate-600 flex gap-3 p-5 rounded-3xl">
            ...
            <TouchableOpacity
              ...
              onPress={() => { deletePatrolProc(); setModalVisible(!modalVisible) }}>
