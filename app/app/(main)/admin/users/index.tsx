...
import { getUserInfo, getEventInfo, getAllUsersInfo, deleteUser } from "@/utils/requests";
...
export default function Users() {
  const updateData = async () => {
    ...
    const usersTmp: any = await getAllUsersInfo();
    if (!usersTmp) {
      router.replace('/');
      return;
    }
  ...
  const deleteUserProc = async () => {
    await deleteUser(toDeleteIdx);
    updateData();
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-400 flex-col items-center gap-5">
      <CustomButton text="Dodaj użytkownika" className="w-96 items-center mt-5 py-5 px-10 bg-slate-600 rounded-2xl" textClassName="text-2xl text-slate-200" onPress={() => router.push('/(main)/admin/users/addUser')} />
      <FlatList data={users} renderItem={({ item }) => {
        return <UserItem onLongPress={() => { setToDeleteIdx(item.id); setModalVisible(true) }} user={item} />
      }}/>
      <Modal
        animationType="slide"
        transparent={true}
        ...
        <View className="flex-1 justify-center items-center">
          <View className="bg-slate-600 flex gap-3 p-5 rounded-3xl">
            ...
            <TouchableOpacity
              ...
              onPress={() => { deleteUserProc(); setModalVisible(!modalVisible) }}>
