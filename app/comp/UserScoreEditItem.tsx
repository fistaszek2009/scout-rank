...
import { changeUserScore } from "@/utils/requests";

export default function UserScoreEditItem(props: { user: any, score: any, update?: Function }) {
    ...
    <View className="flex flex-row gap-5">
      ...
      <View className={"bg-slate-600 w-96 px-5 py-5 mt-5 flex-row justify-center"}>
        <TextInput
          ...
          value={newScore}
        />
        <CustomButton text="Zmień" onPress={() => { changeUserScore(props.user.id, props.score.id, parseInt(newScore)); setNewScore(""); props.update && props.update(); }} />
      </View>
    </View>
  )
}